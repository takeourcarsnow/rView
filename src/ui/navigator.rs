    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .add(
                        egui::Button::new(
                            RichText::new("FIT")
                                .size(10.0)
                                .color(zoom_btn_style(fit_active)),
                        )
                ...
                if ui
                    .add(
                        egui::Button::new(
                            RichText::new("100%")
                                .size(10.0)
                                .color(zoom_btn_style((app.zoom - 1.0).abs() < 0.01)),
                        )
                        .fill(Color32::TRANSPARENT)
                        .stroke(egui::Stroke::NONE)
                ...
                if ui
                    .add(
                        egui::Button::new(RichText::new("25%").size(10.0).color(LR_TEXT_SECONDARY))
                            .fill(Color32::TRANSPARENT)
                            .stroke(egui::Stroke::NONE)
                            .min_size(Vec2::new(28.0, 16.0)),
                    )
                ...
                if app.zoom > 1.0 || app.pan_offset != Vec2::ZERO {
                    ...
                    if response.clicked() {
                        if let Some(click_pos) = response.interact_pointer_pos() {
                            ...
                            let click_fraction_x =
                                (click_pos.x - image_rect.left()) / image_rect.width();
                            let click_fraction_y =
                                (click_pos.y - image_rect.top()) / image_rect.height();
